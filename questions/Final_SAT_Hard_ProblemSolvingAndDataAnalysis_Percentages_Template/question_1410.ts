 ...
 *
 * FIXED (stray backslashes next to percentages — Q1229/Q1239 class):
 * - Round 1 fixed the options; the STEM also had `${P1}\\%` / `${P2}\\%`
 *   sitting in PLAIN TEXT (no math delimiters around them), so the literal
 *   backslash+percent rendered as "2000\%" — same defect, missed in the
 *   first pass. The explanation's final line `$${percentageResult}\\%$` was
 *   also escaped inside math, which this renderer mishandles (the Q1400
 *   bug class: `\%` inside $...$ breaks pairing).
 * - House rule now applied to the WHOLE file: every percent sign is plain
 *   text — never escaped, never inside math delimiters. Fractions and other
 *   commands keep their standard two-in-source backslashes inside $...$.
...
1.  **Set up equations:**
    $a = \\frac{${P1}}{100}(b + c)$     $b = \\frac{${P2}}{100}c$ 
2.  **Solve for $c$ in terms of $b$:**
    $c = \\frac{100b}{${P2}}$ 
3.  **Substitute $c$ into the first equation:**
    $a = \\frac{${P1}}{100} \\left( b + \\frac{100b}{${P2}} \\right)$     
4.  **Factor out $b$:**
    $a = \\frac{${P1}}{100} \\cdot b \\left( 1 + \\frac{100}{${P2}} \\right)$     $a = \\frac{${P1}}{100} \\cdot b \\left( \\frac{${P2} + 100}{${P2}} \\right)$ 
